...
async function generateMetadata(content: string, title: string): Promise<{ 
  ctaCopy: string, 
  ctaButton: string,
  categories: string[]
}> {
  ...
  try {
    ...
    return {
      ctaCopy: '<p>Experience seamless environment management with Release.</p>',
...
async function enhancePostMetadata(filename: string): Promise<void> {
  ...
  try {
    ...
    const { ctaCopy, ctaButton, categories } = await generateMetadata(content, frontmatter.title)
    ...
    if (frontmatter.ctaButton !== ctaButton) {
      changes.push(`- Updating ctaButton from: "${frontmatter.ctaButton || ''}"`)
