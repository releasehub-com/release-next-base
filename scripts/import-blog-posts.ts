  ...
  parse(csvFile, {
    ...
    for (const record of records) {
      try {
        ...
        const now = new Date();
        if (publishDate > now) {
          console.log(`Skipping ${record.Slug} - not yet published (${record['Published On']})`);
          continue;
        }
        ...
        if (mainImagePath) {
          try {
            ...
            console.error(`Failed to download main image for ${record.Slug}:`, err);
          }
        }

        // Convert HTML content to Markdown
        const topContent = record['Post Body Top'] ? await convertHtmlToMarkdown(record['Post Body Top']) : '';
        const bottomContent = record['Post Body Bottom'] ? await convertHtmlToMarkdown(record['Post Body Bottom']) : '';

        // Combine content without injecting CTA component
        const content = [topContent, bottomContent].filter(Boolean).join('\n\n');
        ...
        const frontmatter = {
          ...
          relatedPosts: record['You might also like']?.split(',').map(slug => slug.trim()) || [],
        };
