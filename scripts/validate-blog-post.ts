(() => {
  ...
  async function spellCheckContent(
    content: string,
  ): Promise<{ word: string; suggestions: string[] }[]> {
    if (!dictionary) {
  ...
  function loadAuthors(): Record<string, any> {
    ...
    const authorsMatch = authorsContent.match(
      /export const authors: Record<string, Author> = ({[\s\S]*?});/,
    );
    if (!authorsMatch) {
  ...
  async function validateImage(
    ...
    if (!fs.existsSync(fullPath)) {
      return {
        valid: false,
        issues: [`Image ${imagePath} does not exist`],
        warnings: [],
      };
    }
    ...
    try {
      ...
      if (fileSizeInKB > 500) {
        if (isMainImage) {
          warnings.push(
            `Image size (${Math.round(fileSizeInKB)}KB) exceeds 500KB limit`,
          );
        } else {
          issues.push(
            `Image size (${Math.round(fileSizeInKB)}KB) exceeds 500KB limit`,
      ...
      if (isMainImage && (metadata.width !== 1200 || metadata.height !== 630)) {
        warnings.push(
          `Image dimensions (${metadata.width}x${metadata.height}) differ from recommended 1200x630`,
        );
      }
  ...
  function validateUtmParameters(
    ctaLink: string,
    slug: string,
  ...
  function validateRelatedPosts(relatedPosts: string[]): {
    valid: boolean;
    issues: string[];
  } {
    const issues: string[] = [];
  ...
  function validateFrontmatterFields(frontmatter: BlogFrontmatter): {
    valid: boolean;
    issues: string[];
  } {
    const issues: string[] = [];
    ...
    if (
      ...
      frontmatter.author !== "your-github-username"
    ) {
    ...
    frontmatter.categories.forEach((category) => {
      ...
      if (category.includes(" ")) {
        issues.push(
          `Category "${category}" should use dashes instead of spaces`,
        );
      }
    });
  ...
  async function validateBlogPost(filePath: string): Promise<boolean> {
    ...
    try {
      ...
      if (!fieldsValidation.valid) {
        ...
        fieldsValidation.issues.forEach((issue) =>
          console.error(`   - ${issue}`),
        );
        isValid = false;
      }
      ...
      if (mainImageValidation.warnings.length > 0) {
        ...
        mainImageValidation.warnings.forEach((warning) =>
      ...
      if (frontmatter.ogImage !== frontmatter.mainImage) {
        const ogImageValidation = await validateImage(
          frontmatter.ogImage,
          true,
        );
        if (!ogImageValidation.valid) {
          console.error("❌ OG image validation failed:");
          ogImageValidation.issues.forEach((issue) =>
            console.error(`   - ${issue}`),
          );
          isValid = false;
        }
        if (ogImageValidation.warnings.length > 0) {
          ...
          ogImageValidation.warnings.forEach((warning) =>
            console.warn(`   - ${warning}`),
          );
          hasWarnings = true;
        }
      ...
      if (contentImages.length > 0) {
        ...
        for (const imagePath of contentImages) {
          ...
          if (!imageValidation.valid) {
            console.error(
              `❌ Content image validation failed for ${imagePath}:`,
            );
      ...
      if (!relatedValidation.valid) {
        ...
        relatedValidation.issues.forEach((issue) =>
          console.error(`   - ${issue}`),
        );
        isValid = false;
      }
      ...
      if (wordCount < 300) {
        ...
        console.error(
          `   - Post should be at least 300 words (current: ${wordCount} words)`,
        );
        isValid = false;
      }
      ...
      if (isValid) {
        ...
        if (hasWarnings) {
          console.log(
            "   Note: There are warnings above that you may want to address",
          );
        }
      } else {
        console.log(
