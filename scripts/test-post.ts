...
program
  ...
  .requiredOption('-p, --platform <platform>', 'Platform to post to (twitter or linkedin)')
  .requiredOption('-c, --content <content>', 'Content to post')
  .option('-t, --token <token>', 'Authentication token (if not using session auth)')
  .option('-u, --url <url>', 'Base URL of the application', process.env.NEXTAUTH_URL || 'http://localhost:3000');
