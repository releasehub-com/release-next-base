...
function enhancePostSEO() {
  ...
  mdxFiles.forEach(filename => {
    ...
    if (!frontmatter.readingTime) {
      changes.push(`- Adding estimated reading time: ${estimateReadingTime(content)} minutes`)
    }
    
    if (!frontmatter.ogImage) {
      changes.push(`- Adding OG image: ${frontmatter.mainImage || '/blog/default-og-image.png'}`)
    }
    ...
    enhancedFrontmatter.excerpt = frontmatter.excerpt || frontmatter.summary || generateExcerpt(content)
    enhancedFrontmatter.categories = frontmatter.categories || []
    enhancedFrontmatter.tags = frontmatter.tags || frontmatter.categories || []
    enhancedFrontmatter.readingTime = frontmatter.readingTime || estimateReadingTime(content)
    enhancedFrontmatter.ogImage = frontmatter.ogImage || frontmatter.mainImage || '/blog/default-og-image.png'
