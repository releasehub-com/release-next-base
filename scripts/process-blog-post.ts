    ...
    return blogFiles.map((file) => {
      ...
      if (frontmatterMatch) {
        try {
          ...
          console.log(
            `Warning: Invalid frontmatter in ${file}, treating as content`,
          );
        }
      }
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert at finding related content. Given a blog post and a list of other posts, select the 3 most relevant related posts. Return ONLY the slugs, one per line, without numbers or backticks.",
        },
        {
          role: "user",
  ...
  async function generateFrontmatter(
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (sentenceCount > 2) {
          console.log(
            `Generated summary has ${sentenceCount} sentences. Retrying...`,
          );
          attempts++;
          continue;
        }
    ...
    if (!aiResponse) {
      throw new Error(
        "Failed to generate valid frontmatter after maximum attempts",
      );
    }
    ...
    if (!validateImage(frontmatter.mainImage)) {
      console.warn(
        `Warning: Image ${frontmatter.mainImage} does not exist in the public directory`,
      );
    }
  ...
  async function processBlogPost(
    ...
    if (frontmatterMatch) {
      try {
        ...
        // If YAML parsing fails, treat it as content without frontmatter
        console.log(
          "Invalid frontmatter found, treating as content without frontmatter",
        );
      }
    }
    ...
    const categoriesMatch = frontmatterYaml.match(
      /categories:\n((?:  - .*\n)*)/,
    );
    if (categoriesMatch) {
      const categoriesBlock = categoriesMatch[0];
      const tagsBlock =
        frontmatterYaml.match(/tags:(?:\n(?:  - .*)*)/)?.[0] || "tags: []";
