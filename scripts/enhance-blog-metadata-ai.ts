...
async function generateMetadata(
  ...
  for (let i = 0; i < MAX_RETRIES && !ctaCopy; i++) {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a technical writer who creates precise, clear call-to-action messages for developer tools and platforms. Always follow the exact response format requested.",
        },
        {
          role: "user",
          content: prompt,
    ...
    const ctaMatch = response
      .match(/CTA:\s*\n(.*?)(?=\n\s*CATEGORIES:|$)/s)?.[1]
      ?.trim();
    const categoriesMatch = response
      .match(/CATEGORIES:\s*\n(.*?)(?=\n|$)/s)?.[1]
      ?.trim();
    ...
    if (!ctaCopy) {
      console.log(
        `Retry ${i + 1}: Empty CTA response received for "${title}", retrying...`,
      );
    }
  }
  ...
  if (!ctaCopy) {
    console.warn(
      `Warning: Using fallback CTA for "${title}" after ${MAX_RETRIES} failed attempts`,
    );
...
async function enhancePostMetadata(filename: string) {
  try {
    ...
    const { ctaCopy, ctaButton, ctaLink, categories } = await generateMetadata(
      content,
      frontmatter.title,
      slug,
    );
