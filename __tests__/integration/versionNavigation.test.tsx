...
jest.mock("@/components/GitLabLandingPage", () => () => (
  <div>GitLab Landing</div>
));
jest.mock("@/components/KubernetesLandingPage", () => () => (
  <div>Kubernetes Landing</div>
));
jest.mock("@/components/CloudDevLanding", () => () => (
  <div>Cloud Dev Landing</div>
));
...
jest.mock("@/lib/landingVersions", () => {
  ...
  return {
    ...actual,
    setVersionInStorage: (version: string) =>
      mockLocalStorage.setItem("landing_version", version),
...
describe("Version Navigation Integration", () => {
      ...
      steps: [
        {
          ...
          expectedTitle:
            "Cloud Development Environments for seamless developer experience",
        },
        {
          page: "signup",
          path: "/signup",
      ...
      it("should maintain version and content across navigation", async () => {
        for (const step of scenario.steps) {
          ...
          (useSearchParams as jest.Mock).mockReturnValue(
            step.params || new URLSearchParams(),
          );
          ...
          if (step.page === "landing") {
            expect(
              screen.getByText(step.expectedComponent),
            ).toBeInTheDocument();
          } else {
  ...
  it("should handle invalid version parameter gracefully", async () => {
    (useSearchParams as jest.Mock).mockReturnValue(
      new URLSearchParams("version=invalid"),
    );
    ...
    expect(messageElement).toHaveTextContent(
      "Full-stack on-demand environments",
    );
  });
