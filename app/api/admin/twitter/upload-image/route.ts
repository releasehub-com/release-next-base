...
async function resizeImageIfNeeded(
  ...
  let resizedBuffer = await sharp(buffer).jpeg({ quality }).toBuffer();
...
async function uploadImageToTwitter(
  ...
  if (total_bytes > MAX_FILE_SIZE) {
    throw new Error(
      `Image is too large (${total_bytes} bytes) even after compression. Maximum size is ${MAX_FILE_SIZE} bytes.`,
    );
  }
  ...
  if (finalizeData.processing_info) {
    ...
    while (
      processingData.state === "pending" ||
      processingData.state === "in_progress"
    ) {
      // Wait for the specified check_after_secs
      await new Promise((resolve) =>
        setTimeout(resolve, (processingData.check_after_secs || 1) * 1000),
      );
      ...
      const statusResponse = await fetch(
        `${statusRequestData.url}?${new URLSearchParams(statusRequestData.data).toString()}`,
        {
          headers: {
      ...
      if (processingData.state === "failed") {
        throw new Error(
          `Media processing failed: ${processingData.error?.message || "Unknown error"}`,
        );
      }
    }
  }
  ...
  try {
    ...
    if (!oauth1Creds?.accessToken || !oauth1Creds?.tokenSecret) {
      return NextResponse.json(
        {
          error:
            "Twitter OAuth 1.0a credentials not found. Please connect your account with OAuth 1.0a.",
        },
        { status: 400 },
      );
    }
    ...
    const { media_id } = await uploadImageToTwitter(
      file,
      oauth1Creds.accessToken,
      oauth1Creds.tokenSecret,
    );
