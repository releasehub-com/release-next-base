...
For example, cloud environments created with [Release](https://release.com/) will by default generate a random environment ID, **${ENV\_ID}**,  and let you use that value anywhere you want within the hostname of an ephemeral ingress endpoint, such as https://frontend-**${ENV\_ID}**.example.com.
