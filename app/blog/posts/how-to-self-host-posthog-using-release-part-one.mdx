...
The next steps involve creating build arguments. We always recommend using the **production** value for **NODE\_ENV** since Release ephemeral environments run in production mode, rather than development mode (except when using our [Remote Development](https://docs.release.com/cli/remote-dev) feature).
