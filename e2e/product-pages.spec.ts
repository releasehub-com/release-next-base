...
function shouldSkipImage(
  ...
  const hasSkipClass = className
    ? skipClasses.some((cls) => className.includes(cls))
    : false;
  const isIconNotLogo = className
    ? className.includes("icon") && !src.includes("logo")
    : false;
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      ...
      if (isSvg) {
        ...
        const inViewport = await isInViewport(
          page,
        ...
        if (
          isAvatar ||
          (imageState.loading === "lazy" && !imageState.isInViewport)
        ) {
          console.log(
            `ℹ️ Skipping detailed checks for ${isAvatar ? "avatar" : "lazy-loaded"} image: ${src}`,
          );
          continue;
        }
...
function logRequestSummaries(
  failedRequests: Map<string, any>,
  responseStatuses: Map<string, any>,
) {
...
test.describe("Product Pages", () => {
  ...
  test.describe("Instant Datasets Page", () => {
    test("should render correctly", async ({ page }) => {
      ...
      // Check main heading
      await expect(
        page.getByRole("heading", { name: /Release Instant Datasets/i }),
      ).toBeVisible();

      // Check CTA buttons
      const ctaButtons = page.getByRole("link", {
        name: /Try Instant Datasets Free/i,
      });
      ...
      // Check steps section
      await expect(
        page.getByRole("heading", { name: /Production-like data in 3 steps/i }),
      ).toBeVisible();
      ...
      await expect(
        page.getByText(/Build and test with your data/i),
      ).toBeVisible();
