...
test.describe("SEO Pages", () => {
  ...
  test("should render investor logos correctly on whyrelease page", async ({ page }) => {
    await page.goto('/whyrelease');
    await page.waitForSelector('h2:text-is("Our Investors")');

    // Find the investors section
    const investorsSection = await page.locator('h2:text-is("Our Investors")').locator('..').locator('..');
    
    // Get all investor logo containers within the section
    const logoContainers = await investorsSection.locator('.w-\\[180px\\].h-\\[80px\\]').all();
    expect(logoContainers.length).toBe(3);

    // Expected logos with their properties
    const expectedLogos = [
      { src: '/investors/sequoia.svg', alt: 'Sequoia Capital logo', inverted: false },
    ...
    for (let i = 0; i < logoContainers.length; i++) {
      ...
      expect(src, `Logo ${i + 1} should have correct src`).toContain(expected.src);
      ...
      if (expected.inverted) {
        const hasInvertClass = await image.evaluate((img) => {
          return img.className.includes('brightness-0') && img.className.includes('invert');
        });
        expect(hasInvertClass, `${expected.alt} should have invert classes`).toBe(true);
      }
